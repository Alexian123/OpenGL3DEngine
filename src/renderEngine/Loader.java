...
public class Loader {
	...
	public FontType loadFont(String fontName) {
		return new FontType(loadFontAtlas(fontName), new File("res/fonts/" + fontName + ".fnt"));
	}
	
	// for normal mapping
	public TexturedModel loadTexturedModelNM(String modelName, String textureName) {
		return new TexturedModel(NormalMappedObjLoader.loadOBJ(modelName, this), new ModelTexture(loadTexture(textureName)));
	}
	
	public TexturedModel loadTexturedModel(String modelName, String textureName) {
		ModelData modelData = OBJFileLoader.loadOBJ(modelName);
		RawModel rawModel = loadToVAO(modelData.getVertices(), modelData.getTextureCoords(), 
				modelData.getNormals(), modelData.getIndices());
		return new TexturedModel(rawModel, new ModelTexture(loadTexture(textureName)));
	}
	...
	public void updateVBO(int vbo, float[] data, FloatBuffer buff) {
		buff.clear();
		buff.put(data);
		buff.flip();
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vbo);
		GL15.glBufferData(GL15.GL_ARRAY_BUFFER, buff.capacity() * 4, GL15.GL_STREAM_DRAW);
	...
	public int loadTexture(String fileName) {
		...
		try {
			...
			GL11.glTexParameterf(GL11.GL_TEXTURE_2D, GL14.GL_TEXTURE_LOD_BIAS, -0); // -2.4f
	...
	public int loadFontAtlas(String fileName) {
		...
		try {
			texture = TextureLoader.getTexture("PNG",
					new FileInputStream("res/fonts/" + fileName + ".png"));
			GL30.glGenerateMipmap(GL11.GL_TEXTURE_2D);
			GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR_MIPMAP_LINEAR);
			GL11.glTexParameterf(GL11.GL_TEXTURE_2D, GL14.GL_TEXTURE_LOD_BIAS, -0); // original -0.4f
		} catch (Exception e) {
			e.printStackTrace();
			System.err.println("Tried to load texture " + fileName + ".png , didn't work");
			System.exit(-1);
