package terrains;



import java.util.Random;

import toolbox.Settings;

public class HeightsGenerator {
	
	private Random random = new Random();
	private int seed;
	private int xOffset = 0;
	private int zOffset = 0;
	
	public HeightsGenerator() {
		this.seed = random.nextInt(1_000_000_000); 
	}
	
	public HeightsGenerator(int gridX, int gridZ, int vertexCount, int seed) {
		this.seed = seed;
		xOffset = gridX * (vertexCount-1);
		zOffset = gridZ * (vertexCount-1);
	}
	
	public float generateHeight(int x, int z) {
		float total = 0;
		float d = (float) Math.pow(2, Settings.TERRAIN_OCTAVES - 1);
		for (int i = 0; i < Settings.TERRAIN_OCTAVES; ++i) {
			float freq = (float) (Math.pow(2, i) / d);
			float amp = (float) Math.pow(Settings.TERRAIN_ROUGHNESS, i) * Settings.TERRAIN_AMPLITUDE;
			total += getInterpolatedNoise((x + xOffset) * freq, (z + zOffset) * freq) * amp;
		}
		return total;
	}
	
	private float interpolate(float a, float b, float blend) {
		double theta = blend * Math.PI;
		float g = (float) (1f - Math.cos(theta)) * 0.5f;
		return a * (1f - g) + b * g;
	}
	
	private float getInterpolatedNoise(float x, float z) {
		int intX = (int) x;
		int intZ = (int) z;
		float fractX = x - intX;
		float fractZ = z - intZ;
		
		float v1 = getSmoothNoise(intX    , intZ);
		float v2 = getSmoothNoise(intX + 1, intZ);
		float v3 = getSmoothNoise(intX    , intZ + 1);
		float v4 = getSmoothNoise(intX + 1, intZ + 1);
		float i1 = interpolate(v1, v2, fractX);
		float i2 = interpolate(v3, v4, fractX);
		return interpolate(i1, i2, fractZ);
	}
	
	private float getSmoothNoise(int x, int z) {
		float corners = (makeSomeNoise(x-1, z-1) + makeSomeNoise(x+1, z-1) + makeSomeNoise(x-1, z+1) 
			+ makeSomeNoise(x+1, z+1)) / 16f;
		float sides = (makeSomeNoise(x-1, z) + makeSomeNoise(x+1, z) + makeSomeNoise(x, z-1) + makeSomeNoise(x, z+1)) / 8f;
		float center = makeSomeNoise(x, z) / 4f;
		return corners + sides + center;
	}
	
	private float makeSomeNoise(int x, int z) {
		random.setSeed(x * 49632 + z * 325176 + seed);
		return random.nextFloat() * 2f - 1f;
	}

}
